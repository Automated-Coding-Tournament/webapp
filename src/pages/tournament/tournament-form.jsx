...
const TournamentForm = (props) => {
  ...
  const { response, loading, error, update, clearError } = useUpdate(
    `${dataContext.API}/tournament/edit/${id}`
  );
  const { response: findTasksResponse, find: findTasks } = useFind(
    `${dataContext.API}/task/getByTournament/${id}`
  );
  const { response: findTournamentResponse, find: findTournament } = useFind(
    `${dataContext.API}/tournament/get/${id}`
  );
  const { response: findAllTasksResponse, find: findAllTasks } = useFind(
    `${dataContext.API}/task/getAll`
  );
  const {
    response: saveResponse,
    loading: saveLoading,
    save
  } = useSave(`${dataContext.API}/tournament/create`);
  ...
  const checkForErrors = () => {
    ...
    if (tournament.firstPlacePoints?.value === '') {
      ...
      tournament.firstPlacePoints?.value < 5 ||
      tournament.firstPlacePoints?.value > 100
    ) {
      validationErrors.push({
        propertyName: 'firstPlacePoints',
    ...
    if (tournament.secondPlacePoints?.value === '') {
      ...
      tournament.secondPlacePoints?.value < 5 ||
      tournament.secondPlacePoints?.value > 100
    ) {
      validationErrors.push({
        propertyName: 'secondPlacePoints',
        errorMessage: 'Please enter a valid value in range [5; 100]'
    ...
    if (tournament.thirdPlacePoints?.value === '') {
      ...
      tournament.thirdPlacePoints?.value < 5 ||
      tournament.thirdPlacePoints?.value > 100
    ) {
      validationErrors.push({
        propertyName: 'thirdPlacePoints',
        errorMessage: 'Please enter a valid value in range [5; 100]'
  ...
  return (
    <Container>
      <Form
        title={id && id !== 'new' ? 'Edit tournament' : 'Create tournament'}
